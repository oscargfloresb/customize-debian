...
# Debian Shell Theme
#
# A clean and modern Debian-style prompt
#
# Author  : OKY
# Version : 1.0
# License : GNU GPL v3
#
# Inspired by the Kali Linux zsh prompt and modern terminal workflows.
#
# Recommended for the best Debian Shell experience:
#   apt install zsh-syntax-highlighting zsh-autosuggestions — enables
#   syntax highlighting, command autosuggestions and interactive features.
#
#############################################################################
...
_debian_async_vcs_tmpfile="/tmp/.debian_vcs_info_$$"

_debian_async_vcs() {
    vcs_info
    printf '%s' "${vcs_info_msg_0_}" > "$_debian_async_vcs_tmpfile"
    kill -USR1 $$ 2>/dev/null
}

TRAPUSR1() {
    if [[ -f "$_debian_async_vcs_tmpfile" ]]; then
        vcs_info_msg_0_="$(<"$_debian_async_vcs_tmpfile")"
...
_debian_zshexit() {
    rm -f "$_debian_async_vcs_tmpfile"
}

autoload -Uz add-zsh-hook
add-zsh-hook precmd _debian_precmd
add-zsh-hook preexec _debian_preexec
add-zsh-hook zshexit _debian_zshexit
